...
def calcRanking():
    sql="""
        ...
        (
            select (SUM(case when voldaan='V' then 1 else 0 end))*100+round(100*sum(punten/(punten_max+1)) 'Ranking Score'
            FROM resultaat r
    ...
    sql="""
        UPDATE resultaat t1
        INNER JOIN ( SELECT r.student_nummer, module_id, round(sum( round(d.norm_uren*(r.punten*10/(r.punten_max+1))  )/10) sum_norm_uren
            FROM resultaat r
