...
DELIMITER $$
CREATE OR REPLACE PROCEDURE updateVoldaan(IN in_module_id INT, IN in_student_nr INT, IN in_criteria VARCHAR(200))
BEGIN
    update resultaat set voldaan = 'V' WHERE module_id=in_module_id and $voldaan_criteria[$module_id] and student_nummer=in_student_nr;
END$$


DELIMITER $$
CREATE OR REPLACE PROCEDURE updateVoldaan()
...
BEGIN
  ...
  LOOP
     update resultaat set voldaan = 'V' WHERE module_id=regels.id and $voldaan_criteria[$module_id] and student_nummer=in_student_nr;
    END LOOP;
END;